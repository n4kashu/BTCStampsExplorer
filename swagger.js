...
async function extractJsdocComments(filePath) {
  ...
  const comments = [...content.matchAll(/\/\*\*([\s\S]*?)\*\//g)].map(
    (match) => {
...
async function processYamlFile(filePath) {
  ...
  let processedLines = lines.map((line, index) => {
    ...
    if (line.trim().startsWith("description:")) {
      const descriptionContent = line.trim().substring("description:".length)
        .trim();
...
async function main() {
  ...
  const openApiSpec = {
    ...
    servers: [
      { url: "https://stampchain.io/api/v3" },
  ...
  for await (
    const entry of walk(routesDir, { includeDirs: false, exts: [".ts"] })
  ) {
